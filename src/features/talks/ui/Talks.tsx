...
export const Talks: React.FC = () => {
  ...
  const { hours, talks, halls } = useGetTalks({
    topicSelected,
    speakerSelected,
  });

  const tableCells = (hour: string, hall: Hall) => {
    const talkByHourHall = talks.filter(
      (talk) => talk.hall.id === hall.id && talk.hour.start === hour
    );
    return talkByHourHall.length > 0 ? (
      <td
        key={hour + hall.id}
        rowSpan={getHoursDuration(
          talkByHourHall[0].hour.start,
          talkByHourHall[0].hour.end
        )}
      >
    ...
    <section className="talks">
      <div className="talks-container">
        ...
        <div className="selectors">
          <TopicsSelector
            setSelectedTopic={(topic: number) => setTopicSelected(topic)}
          />
          <SpeakersSelector
            setSelectedSpeaker={(speaker: number) =>
